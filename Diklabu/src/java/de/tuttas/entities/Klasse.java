...
@NamedQueries({
   ...
   @NamedQuery(name = "findKlassebySchuelerID", query= "select k from Klasse k JOIN Schueler_Klasse sk ON k.ID=sk.ID_KLASSE  where sk.ID_SCHUELER= :paramIDSchueler and k.ID_KATEGORIE=0"),
   @NamedQuery(name = "findKlasseByLehrerId", query= "select k from Klasse k  where k.ID_LEHRER= :paramLehrerId"),
   @NamedQuery(name = "findAllKlassen", query= "select NEW de.tuttas.restful.Data.KlasseShort(k.ID,k.ID_LEHRER,k.KNAME,k.ID_KATEGORIE) from Klasse k ORDER BY k.KNAME"),
   ...
   @NamedQuery(name = "findKlasse", query= "select k from Klasse k where k.ID_KATEGORIE =:paramKategorieId"),        
