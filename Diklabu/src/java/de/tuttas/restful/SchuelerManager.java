...
public class SchuelerManager {
    ...
    public StreamingOutput getPdf(@PathParam("id") int id) throws Exception {
        ...
        return new StreamingOutput() {
            public void write(OutputStream out) throws IOException {
                ...
                Query q2 = em.createNamedQuery("findPortfolioBySchueler");
                q2.setParameter("paramIdSchueler", sid);
                ...
                try {
                    ...
                    javax.ws.rs.core.Response.ResponseBuilder responseBuilder = javax.ws.rs.core.Response.ok((Object) document);
                    ...
                    Log.d("Documentexception:" + ex.getMessage());
     ...
     *
     * @param toFind Zeichenkette aus Vorname NachName und Geburtsdatum (z.B.
     * ThomasMustermann1993-12-01)
    ...
    @Produces({"application/json; charset=iso-8859-1"})
    @Path("{tofind}/{ldist}")
    public List<SchuelerObject> searchSchueler(@PathParam("tofind") String toFind,
            @PathParam("ldist") int lDist
    ) {
    ...
    public Schueler setPupil(@PathParam("idschueler") int sid, Schueler s
        ...
        if (sl != null) {
            if (s.getABGANG() != null) {
                sl.setABGANG(s.getABGANG());
            }
            if (s.getEMAIL() != null) {
                sl.setEMAIL(s.getEMAIL());
            }
            if (s.getGEBDAT() != null) {
                sl.setGEBDAT(s.getGEBDAT());
            }
            if (s.getID_AUSBILDER() != null) {
                sl.setID_AUSBILDER(s.getID_AUSBILDER());
            }
            if (s.getINFO() != null) {
                sl.setINFO(s.getINFO());
            }
            if (s.getNNAME() != null) {
                sl.setNNAME(s.getNNAME());
            }
            if (s.getVNAME() != null) {
                sl.setVNAME(s.getVNAME());
            }
            if (s.getID_MMBBS() != null) {
                sl.setID_MMBBS(s.getID_MMBBS());
            }
            em.merge(sl);
        }
        return sl;
